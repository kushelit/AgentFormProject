...
const AgencySummaryAgentsPage: React.FC = () => {
  ...
  // מצב תצוגת סוכנים – כמו בעמוד עמלות
  ...
  // כשנטענים סוכנים – מסמנים את כולם
  useEffect(() => {
  ...
  // סוכנים לפני סינון "נבחרים בלבד"
  const visibleAgents = useMemo(() => agents, [agents]);

  // סוכנים לפי חיפוש – לרשימת ה-checkbox
  ...
  // 🔹 דאטה לגרפים – מבוסס על אותם totals לכל סוכן
  const graphsData = useMemo(
    () =>
      filteredAgents.map((agent) => {
        const t = agentTotalsMap[agent.id] || emptyTotals;
        return {
          agentId: agent.id,
          agentName: agent.name,
          finansimTotal: t.finansimTotal,
          pensiaTotal: t.pensiaTotal,
          insuranceTotal: t.insuranceTotal,
          commissionHekefTotal: t.commissionHekefTotal,
          commissionNifraimTotal: t.commissionNifraimTotal,
        };
      }),
    [filteredAgents, agentTotalsMap]
  );

  // פעולות על בחירת סוכנים
    ...
    <div className="content-container-NewAgentForm" dir="rtl">
      <div className="table-container-AgentForm-new-design">
        <div className="table-header">
          <div className="table-title">דף מרכז – סיכומי סוכנות לפי סוכן</div>
          <div className="text-xs text-gray-600 mt-1">
            כל שורה מייצגת סוכן אחד – סיכום שנתי לכל אחד מסוגי התפוקה והעמלות.
          </div>
        </div>

        {/* 🔹 שורת פילטרים ברמת סוכנות */}
        ...
        {/* 🔹 גרפים – כמו בעמוד עמלות */}
        {graphsData.length > 0 && (
          <div className="mt-8 space-y-8">
            {/* תפוקה שנתית לפי סוכן */}
            <div>
              <h3 className="text-sm font-semibold mb-2">
                תפוקה שנתית לפי סוכן (פיננסים / פנסיה / ביטוח)
              </h3>
              <div style={{ width: '100%', height: 320 }}>
                <ResponsiveContainer>
                  <BarChart data={graphsData}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="agentName" />
                    <YAxis />
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="finansimTotal" name="פיננסים" />
                    <Bar dataKey="pensiaTotal" name="פנסיה" />
                    <Bar dataKey="insuranceTotal" name="ביטוח" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            {canViewCommissions && (
              <div>
                <h3 className="text-sm font-semibold mb-2">
                  עמלות שנתיות לפי סוכן (היקף / נפרעים)
                </h3>
                <div style={{ width: '100%', height: 320 }}>
                  <ResponsiveContainer>
                    <BarChart data={graphsData}>
                      ...
                      <Legend />
                      <Bar
                        dataKey="commissionHekefTotal"
                        name="עמלת היקף"
                      />
                      <Bar
                        dataKey="commissionNifraimTotal"
                        name="עמלת נפרעים"
                      />
                    </BarChart>
                  </ResponsiveContainer>
...
export default AgencySummaryAgentsPage;
