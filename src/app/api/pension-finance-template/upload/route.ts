...
const COL = {
  ...
  PENSIA_PREMIA: 'פרמיית פנסיה',
  PENSIA_ZVIRA: 'צבירת פנסיה',
  FINANSIM_PREMIA: 'פרמיית פיננסים',
  FINANSIM_ZVIRA: 'צבירת פיננסים',
  NOTES: 'הערות',
  ...
  REFERRER: 'נציג מפנה',
  KUPA_ACTION: 'פעולה',
  TRANSFER_COMPANY: 'חברה לניוד',
  TRANSFER_KUPA_TYPE: 'סוג קופה לניוד',
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sheet = workbook.getWorksheet("עסקאות - פנסיה ופיננסים") || workbook.worksheets[0];
    if (!sheet) {
    ...
    sheet.eachRow((row, rowNumber) => {
      ...
      if (!matchedProduct) errors.push(`מוצר לא מזוהה, או שאינו שייך לקבוצת פנסיה/פיננסים: "${productName}"`);
      ...
      const parseOptionalNumber = (colName: string): { value: number; error?: string } => {
        if (!colIndex[colName]) return { value: 0 };
        const raw = getCellText(row, colIndex[colName]);
        if (!raw) return { value: 0 };
        const n = Number(raw);
        if (isNaN(n)) return { value: 0, error: `ערך לא מספרי בעמודה "${colName}": "${raw}"` };
        return { value: n };
      };

      const pensiaPremiaR = parseOptionalNumber(COL.PENSIA_PREMIA);
      const pensiaZviraR = parseOptionalNumber(COL.PENSIA_ZVIRA);
      const finansimPremiaR = parseOptionalNumber(COL.FINANSIM_PREMIA);
      const finansimZviraR = parseOptionalNumber(COL.FINANSIM_ZVIRA);
      [pensiaPremiaR, pensiaZviraR, finansimPremiaR, finansimZviraR].forEach((r) => { if (r.error) errors.push(r.error); });
      ...
      let kupaAction = "";
      let transferCompany = "";
      let transferKupaType = "";
      let kupaStatus = "";
      let candidateStatus = "";
      if (isAgency4) {
        referrerName = colIndex[COL.REFERRER] ? getCellText(row, colIndex[COL.REFERRER]) : "";
        kupaAction = colIndex[COL.KUPA_ACTION] ? getCellText(row, colIndex[COL.KUPA_ACTION]) : "";
        kupaStatus = colIndex[COL.KUPA_STATUS] ? getCellText(row, colIndex[COL.KUPA_STATUS]) : "";
        candidateStatus = colIndex[COL.CANDIDATE_STATUS] ? getCellText(row, colIndex[COL.CANDIDATE_STATUS]) : "";

        if (referrerName && !referrerNames.includes(referrerName)) errors.push(`נציג מפנה לא מזוהה: "${referrerName}"`);
        if (kupaAction && kupaAction !== 'קופה חדשה' && kupaAction !== 'ניוד') errors.push(`פעולה לא תקינה: "${kupaAction}"`);
        if (kupaStatus && kupaStatus !== 'פעיל' && kupaStatus !== 'לא פעיל') errors.push(`סטטוס קופה לא תקין: "${kupaStatus}"`);
        if (candidateStatus && candidateStatus !== 'עצמאי' && candidateStatus !== 'שכיר') errors.push(`סטטוס מועמד לא תקין: "${candidateStatus}"`);

        if (kupaAction === 'ניוד') {
          transferCompany = colIndex[COL.TRANSFER_COMPANY] ? getCellText(row, colIndex[COL.TRANSFER_COMPANY]) : "";
          transferKupaType = colIndex[COL.TRANSFER_KUPA_TYPE] ? getCellText(row, colIndex[COL.TRANSFER_KUPA_TYPE]) : "";
          if (transferCompany && !companyNames.includes(transferCompany)) errors.push(`חברה לניוד לא מזוהה: "${transferCompany}"`);
          if (transferKupaType && !products.some((p) => p.name === transferKupaType)) {
            errors.push(`סוג קופה לניוד לא מזוהה, או שאינו שייך לקבוצת פנסיה/פיננסים: "${transferKupaType}"`);
          }
        }
        // ⚠️ אם קופה_action !== 'ניוד' — transferCompany/transferKupaType נשארים ריקים בכוונה,
        // בדיוק כמו handleKupaActionChange ב-DealFormModal.tsx
      }
      ...
      const payload: any = {
        ...
        insPremia: 0,
        pensiaPremia: pensiaPremiaR.value,
        pensiaZvira: pensiaZviraR.value,
        finansimPremia: finansimPremiaR.value,
        finansimZvira: finansimZviraR.value,
        ...
        kupaAction: isAgency4 ? kupaAction : "",
        transferCompany: isAgency4 ? transferCompany : "",
        transferKupaType: isAgency4 ? transferKupaType : "",
        kupaStatus: isAgency4 ? kupaStatus : "",
        candidateStatus: isAgency4 ? candidateStatus : "",
        sourceApp: "pensionFinanceImport",
      };
    ...
    console.error("pension-finance-template upload error:", error);
    return NextResponse.json({ error: "Failed to upload pension-finance template" }, { status: 500 });
  }
}