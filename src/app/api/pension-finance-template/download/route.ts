...
export async function GET(request: Request) {
  try {
    ...
    const { buffer, filename } = await generatePensionFinanceTemplateExcel({
      agentId,
      isAgency4,
      companies,
      products,
    ...
    return new NextResponse(fileBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="pension-finance-template.xlsx"; filename*=UTF-8''${encodeURIComponent(filename)}`,
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    console.error("pension-finance-template download error:", error);
    return NextResponse.json({ error: "Failed to generate pension-finance template" }, { status: 500 });
  }
}