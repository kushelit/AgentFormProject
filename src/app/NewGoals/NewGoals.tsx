...
const tabs = [
  { id: "default", label: "ברירת מחדל", disabled: false },
  { id: "selected", label: "נבחר", disabled: false },
  { id: "disabled", label: "לא זמין", disabled: true },
];
...
const [isProcessing, setIsProcessing] = useState(false); // Track loading state
  ...
  <div className="newGoalButton">
    <button onClick={handleOpenModalNewGoal}>סוג יעד חדש</button>
  </div>
  {/* המודל */}
  {isModalOpenNewGoal && (
    <div className="modal">
      <div className="modal-content">
        <button className="close-button" onClick={handleCloseModalNewGoal}>
          ✖
        </button>
        <h2>סוג יעד חדש</h2>
      ...
      <tbody>
        {promotionList.map((item) => (
          <tr
            ...
            className={`${selectedRowPromotion && selectedRowPromotion.id === item.id
                ? "selected-row"
                : ""
          ...
          >
            ...
            <td>
              {item.promotionStartDate ? formatIsraeliDateOnly(item.promotionStartDate) : ""}
            </td>
            <td>
              {item.promotionEndDate ? formatIsraeliDateOnly(item.promotionEndDate) : ""}
            </td>
...
<div className="NewGoalsStars">
      {/* כפתור לפתיחת המודל */}
      <button onClick={handleOpenModalNewStars}>כוכב חדש</button>
      {/* המודל */}
      {isModalOpenNewStars && (
        <div className="modal">
          <div className="modal-content">
            <button className="close-button" onClick={handleCloseModalNewStars}>
              ✖
            </button>
            <h2>כוכב חדש</h2>
      ...
      {/* כפתור לפתיחת המודל */}
      <button onClick={handleOpenModalGoalWorker}>יעד חדש</button>
      {/* המודל */}
      {isModalOpenGoalWorker && (
        <div className="modal">
          <div className="modal-content">
            <button className="close-button" onClick={handleCloseModalGoalWorker}>
              ✖
            </button>
            <h2>יעד חדש</h2>
