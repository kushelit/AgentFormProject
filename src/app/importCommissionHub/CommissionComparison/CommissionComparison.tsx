...
const CommissionComparison = () => {
  ...
  const [statusFilter, setStatusFilter] = useState(''); // (נשאר אם תרצי להחזיר בהמשך)
  const [agentCodes, setAgentCodes] = useState<string[]>([]);
  const [selectedCompanyId, setSelectedCompanyId] = useState('');
  const [drillStatus, setDrillStatus] = useState<string | null>(null);
  const [templateOptions, setTemplateOptions] = useState<TemplateOption[]>([]);

  // YYYY-MM מקומי כדי לא להיתקע מ-UTC
  ...
  // אתחול: חודש שני = נוכחי, חודש ראשון = קודם
  useEffect(() => {
    setMonth2(todayYm);
    setMonth1(addMonths(todayYm, -1));
  }, []); // פעם אחת ב-mount

  // לשמור שחודש שני לא יקטן מחודש ראשון
  useEffect(() => {
    if (month1 && month2 && month2 < month1) setMonth2(month1);
  }, [month1, month2]);

  // שליפת תבניות + שמות חברות
  ...
  // עדכון רשימת מספרי סוכן לפי הסוכן שנבחר
  useEffect(() => {
  ...
  const total1 = visibleRows.reduce(
    (sum, r) => (typeof r.row1?.commissionAmount === 'number' ? sum + r.row1.commissionAmount : sum),
    0
  );

  const total2 = visibleRows.reduce(
    (sum, r) => (typeof r.row2?.commissionAmount === 'number' ? sum + r.row2.commissionAmount : sum),
    0
  );

  // קומפוננטת בחירת חודש עם חיצים (RTL: קודם=ימינה, הבא=שמאלה)
