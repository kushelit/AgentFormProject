...
export default function CreateSurensePowerOfAttorneyStepEditor({ step, onConfigChange }: { step: FlowStep; onConfigChange: (patch: Record<string, unknown>) => void }) {
    ...
    <div className="space-y-5">
      ...
      <fieldset className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-2 grid gap-3 md:grid-cols-3">
          <Option label="הר הביטוח — נספח ה׳" checked={includeHb} onChange={(checked) => onConfigChange({ includeHb: checked })} />
          <Option label="פוליסות — נספח ב׳" checked={includePolicies} onChange={(checked) => onConfigChange({ includePolicies: checked })} />
          <Option label="מסלקה — נספח א׳" checked={includeSwiftness} onChange={(checked) => onConfigChange({ includeSwiftness: checked })} />
        </div>
      </fieldset>

      <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-sm leading-6 text-emerald-800">
        בשלב WhatsApp הבא השתמשי ב:
        <div className="mt-2 overflow-x-auto rounded-lg border border-emerald-200 bg-white px-3 py-2 font-mono text-xs" dir="ltr">
