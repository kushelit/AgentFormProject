import type { MeslekaPdfReturnRow } from "@/lib/pension/types";
...
function extractTrackSectionLines(allLines: string[]): string[] {
  const startIndex = allLines.findIndex((line: string) => line.includes("פירוט מסלולי השקעה"));
  if (startIndex === -1) return [];

  const endIndex = allLines.findIndex(
    (line: string, idx: number) => idx > startIndex && line.includes("פירוט הרכב נכסים")
  );

  const section = allLines.slice(startIndex + 1, endIndex === -1 ? undefined : endIndex);

  return section.filter((line: string) => {
...
export async function parseMeslekaPdfBuffer(buffer: Buffer): Promise<MeslekaPdfReturnRow[]> {
  ...
  const allLines = rawText
    .split("\n")
    .map((line: string) => normalizeSpaces(line))
    .filter(Boolean);

  const sectionLines = extractTrackSectionLines(allLines);

  return sectionLines
    .map((line: string) => parseTrackLine(line))
