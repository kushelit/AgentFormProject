...
export interface MagicTouchWaitingFor {
  type: MagicTouchWaitingForType;
  stepId?: string | null;
  resumeStepId?: string | null;
  expectedActions?: string[];
  responseOptions?: MagicTouchResponseOption[];
  promptContext?: MagicTouchWaitingPromptContext | null;
  startedAt?: any;
...
export interface MagicTouchFlowRun {
  ...
  attempts?: number;
  waitingFor?: MagicTouchWaitingFor | null;
...
export interface MagicTouchAgentContext {
  booking: MagicTouchAgentBookingContext;
}

export interface MagicTouchExecutionContext {
  agentId: string;
  run: MagicTouchFlowRun;
  flow: MagicTouchFlowDefinitionV2;
  event: Record<string, any>;
  contact?: Record<string, any>;
  agent?: MagicTouchAgentContext;
}
