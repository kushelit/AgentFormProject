...
const commissionOverrides: Record<string, (row: any) => number> = {
  ...
  menura_new_nifraim: (row) =>
    ...
    // מאגד-סכום עמלה - קיימת רק בדוחות שנקלטו לפי מספר הנהלת חשבונות
    // (בית סוכן). בדוחות רגילים העמודה לא קיימת בקובץ בכלל, אז pick()
    // מחזירה undefined ו-toNum הופכת ל-0 - בטוח להוסיף תמיד, בלי תנאי.
  ...
  menura_new_zvira: (row) =>
    ...
    // מאגד-סה'כ עמלה - כנ"ל, רלוונטי רק לריצות הנהלת-חשבונות
    toNum(
      pick(row, [
        "מאגד-סה'כ עמלה",
        "מאגד - סה'כ עמלה",
