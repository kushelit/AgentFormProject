...
async function downloadStorageFileToTmp(params: {bucketNameRaw: string; storagePath: string}) {
  ...
  throw new Error(`Download failed for all buckets: ${JSON.stringify(candidates)} :: ${String(lastErr?.message || lastErr)}`);
}
...
export const processCommissionImportQueue = onDocumentWritten(
  ...
  async (event) => {
    ...
    // ====== 1) CLAIM ATOMIC ======
    const now = Date.now();
    const leaseMs = 10 * 60 * 1000; // 10 דקות
    ...
    const claimed = await db.runTransaction(async (tx) => {
      ...
      // רק queued מותר להתחיל
      // אם processing אבל ה-lease פג, מותר "לחלץ" ולהמשיך
      ...
      // אם כבר יש finishedAt — לא נוגעים
      ...
      tx.set(
        queueRef,
        {
          ...
          lockedAt: nowTs(),
          leaseUntil: new Date(leaseUntilMillis), // Firestore ימיר ל-Timestamp
    ...
    // ====== 2) DO WORK (IDEMPOTENT EXPECTATION) ======
    ...
    try {
      if (!templateId || !agentId || !companyId || !storagePath) {
        throw stepError("validate_job", "Missing required job fields", {templateId, agentId, companyId, storagePath});
      }
      ...
      const agentName = userSnap.exists ? safeStr(userSnap.data()?.name || userSnap.data()?.fullName || userSnap.data()?.displayName) : "";
      ...
      if (path.extname(storagePath).toLowerCase() === ".zip") {
        ...
        if (!picked.ok) throw stepError("zip_select", picked.reason, {candidates: picked.candidates, zipEntryRules});
      ...
      // ⚠️ חשוב: commitRun צריך להיות אידמפוטנטי לפי runId
      // (למשל כתיבה למסמכים עם IDs דטרמיניסטיים לפי runId+rowIndex וכו')
